...
class CosmoAgent:
    ...
    def __init__(self):
        ...
        print(cf.bold | cf.purple ("VALERA loading....... "))
    ...
    def run(self):
        ...
        while True:
            ...
            if situation_narrative == "":
                situation_narrative = "Valera is having a friendly conversation with a friend."
            role_instruction = input(cf.orange("Which role should Valera take? Who is Cosmo talking to? (You can leave it empty):"))
            if role_instruction == "":
                role_instruction = "You are Valera and you are talking to a friend."
        ...
        print(cf.blue("Valera: See you!"))

    def chat(self, situation_narrative, role_instruction):
        print(cf.green("Chat with Valera! Input [RESET] to reset the conversation history and [END] to end the conversation."))
        while True:
            user_input = input("You: ")
            if user_input == "[RESET]":
                self.reset_history()
                print(cf.green("[Conversation history cleared. Chat with Valera!]"))
                continue
            if user_input == "[END]":
                self.reset_history()
                break
...
def main():
    print(cf.bold | cf.blue("Welcome to Valera!"))
    cosmo = CosmoAgent()
    cosmo.run()
